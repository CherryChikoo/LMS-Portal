...
function LeaderboardContent() {
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8 font-sans">
      ...
      {/* Filters (Admin Only) */}
      {userRole !== "student" && (
        <motion.div variants={staggerItem} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl bg-card border border-border/50">
      ...
      {rankedStudents.length === 0 ? (
        <motion.div variants={staggerItem} className="p-12 text-center bg-card/50 rounded-2xl border border-border/50">
